...
import {
  ...
  IconMusic, IconPlay, IconPause, IconGift, IconZap, IconLock,
  IconSend, IconMic, IconCheckCheck, IconArrowRight, IconArrowLeft,
  IconPhone, IconVideo, IconMore, IconSun, IconMoon, IconHeart,
  IconSparkRm, IconStar, IconShare, IconClock, IconCard, IconMessage,
} from './core/icons'
...
// Self-edit (ajuste da música/prévia pelo cliente) — extraído p/ reuso no PaymentPage
...
async function apiCreateOrder(body) {
  ...
  try {
    if (typeof window !== 'undefined' && /(^|\.)staging\./.test(window.location.hostname)) {
      ...
      if (['control', 'p2990', 'p2900', 'p29', 'p37', 'p47', 'p67'].includes(pv)) _fv = { forceVariant: pv }
    }
  } catch (_) {}
...
// ShareButton foi extraído para ./components/ShareButton (reuso em Minhas Músicas
// + DeliveryPage /p/). Importado no topo deste arquivo.
            ...
            <div className="unlocked-downloads">
              {(resultData?.original_url || resultData?.preview_url) && (
                <a className="unlocked-btn" href={resultData.original_url || resultData.preview_url}
                  download={`lembrancacantada-${(resultData.honoreeName || 'musica').toLowerCase()}.mp3`}>
              ...
              {resultData?.video_url && (
                <a className="unlocked-btn" href={resultData.video_url}
                  download={`lembrancacantada-${(resultData.honoreeName || 'video').toLowerCase()}.mp4`}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
                  </svg>
                  Baixar vídeo
